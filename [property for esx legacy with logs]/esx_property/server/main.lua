...
AddEventHandler('esx_property:getItem', function(owner, type, item, count)
	...
	elseif type == 'item_weapon' then
		TriggerEvent('esx_datastore:getDataStore', 'property', xPlayerOwner.identifier, function(store)
			...
			dclog(xPlayer, '**Person Add** X'..weaponName..'** **'..ammo..' **From The Apartment**')
		end)
	end
end)
	...
	elseif type == 'item_weapon' then
		if xPlayer.hasWeapon(item) then
			...
			TriggerEvent('esx_datastore:getDataStore', 'property', xPlayerOwner.identifier, function(store)
				...
				store.set('weapons', storeWeapons)
			 dclog(xPlayer, '**Person Removed Ammo ** X'..ammo..'** **'..name..' **From The Apartment**')
...
function dclog(xPlayer, text, xTarget, source, to, amount)
    ...
    local data = {
      ["username"] = "DracoCustom",
      ["avatar_url"] = "https://imgur.com/8zBSJLV.png",
      ["embeds"] = {{
        ["author"] = {
